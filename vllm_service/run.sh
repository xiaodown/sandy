...
VLLM_BRAIN_LIMIT_MM_PER_PROMPT="${VLLM_BRAIN_LIMIT_MM_PER_PROMPT:-image=10}"
VLLM_BRAIN_MM_PROCESSOR_KWARGS="${VLLM_BRAIN_MM_PROCESSOR_KWARGS:-}"
...
case "$profile_lc" in
  ...
  qwen3)
    ...
    if [[ -z "$VLLM_BRAIN_DEFAULT_CHAT_TEMPLATE_KWARGS" ]]; then
      VLLM_BRAIN_DEFAULT_CHAT_TEMPLATE_KWARGS='{"enable_thinking":false}'
    fi
    if [[ "$model_lc" == *qwen3.6* ]] && is_enabled "$VLLM_BRAIN_ENABLE_MTP"; then
      if [[ -z "$VLLM_BRAIN_SPECULATIVE_CONFIG" ]]; then
        VLLM_BRAIN_SPECULATIVE_CONFIG='{"method":"mtp","num_speculative_tokens":1}'
      fi
    fi
    ;;
  qwen3-nvfp4)
    ...
    if is_enabled "$VLLM_BRAIN_ENABLE_MTP"; then
      if [[ -z "$VLLM_BRAIN_SPECULATIVE_CONFIG" ]]; then
        VLLM_BRAIN_SPECULATIVE_CONFIG='{"method":"mtp","num_speculative_tokens":2}'
      fi
    fi
    ;;
  llama)
...
export VLLM_LIMIT_MM_PER_PROMPT="image=10"
...
append_arg --limit-mm-per-prompt "$VLLM_BRAIN_LIMIT_MM_PER_PROMPT"
append_arg --mm-processor-kwargs "$VLLM_BRAIN_MM_PROCESSOR_KWARGS"
